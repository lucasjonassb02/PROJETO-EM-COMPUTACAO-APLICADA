...
# --- NOVA ROTA ADICIONADA PARA TRATAR O UPLOAD CORRETAMENTE ---
...
def alterar_foto():
    ...
    if 'foto_nova' in request.files:
        ...
        if arquivo.filename != '':
            ...
            # Força o uso de barras normais para não quebrar no HTML do navegador
            usuarios_db[username_logado]['foto'] = f'/static/uploads/{nome_seguro}'
...
def relatorios():
    ...
    usuario_dados = usuarios_db.get(session['usuario'])
    # Listar logs de todos, exceto ações puras do sistema ou se preferir ver tudo remova a condicional
    return render_template('relatorios.html', logs=logs_atividade, user=usuario_dados)
